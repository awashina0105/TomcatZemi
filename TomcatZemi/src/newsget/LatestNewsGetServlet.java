...
/**
 * Servlet implementation class LatestNewsGetServlet
 */
@WebServlet("/LatestNewsGetServlet")
public class LatestNewsGetServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public LatestNewsGetServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		latestNewsGetArray = lngdao.latestNewsGet(today);


		if (latestNewsGetArray.getArrayListSize() != 0){
			session.setAttribute("latestNews", latestNewsGetArray);
			send ="最新お知らせ表示画面";
		}
